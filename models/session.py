...
def fetch_file(session_id: str, rel_path: str) -> None:
    """Download a single file from GCS to local cache.

    Local backend: no-op.
    GCP backend: downloads the file unconditionally, overwriting any stale local copy.
    Silent no-op if the file does not exist in GCS.
    """
